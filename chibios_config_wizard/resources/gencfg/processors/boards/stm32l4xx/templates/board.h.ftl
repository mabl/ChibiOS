...
${line + ")"}
  ...
  [#list port.* as pin]
    ...
    [#assign speed = pin.@Speed[0] /]
    [#if speed == "Minimum"]
      [#assign out = "PIN_OSPEED_LS(" + port_name + "_" + name + ")" /]
    [#elseif speed == "Low"]
      [#assign out = "PIN_OSPEED_MS(" + port_name + "_" + name + ")" /]
    [#elseif speed == "High"]
      [#assign out = "PIN_OSPEED_FS(" + port_name + "_" + name + ")" /]
    [#else]
      [#assign out = "PIN_OSPEED_HS(" + port_name + "_" + name + ")" /]
    [/#if]
    [#if pin_index == 0]
