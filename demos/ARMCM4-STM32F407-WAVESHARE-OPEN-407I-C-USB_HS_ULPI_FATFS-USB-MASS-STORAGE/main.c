...
static WORKING_AREA(WA_sdcard_thread, 512);
msg_t sdcard_thread_sdio_test(void *arg)
{
  (void) arg;
  uint32_t errors;
  uint32_t bytes_written;
  FRESULT fr;
  static FIL sd_file_handle;

  BaseSequentialStream *chp = (BaseSequentialStream*)&SD2;

  chRegSetThreadName("sdcard_test");
  chprintf(chp,"SD Test Thread\r\n");

  // all io pin settings should be in board.h


  /*
  if( init_sd() != 0 ) {
      goto sd_test_cleanup;
  }
  */


  chprintf(chp, "Mounting fatfs\r\n");
  fr = f_mount(0, &SDC_FS);
  if( fr != FR_OK )
  {
    chprintf(chp, "Failed to mount fatfs\r\n");
    goto sd_test_cleanup;
  }

  chprintf(chp, "Opening test.txt\r\n");
  fr = f_open(&sd_file_handle, "/test.txt", FA_CREATE_ALWAYS | FA_WRITE);
  if( fr != FR_OK )
  {
    chprintf(chp, "Failed to open test.txt, %d\r\n", fr);
    goto sd_test_cleanup;
  }

  chprintf(chp, "Writing to test.txt\r\n");
  int i;
  for(i = 0; i < 1000000; i++ ) {
    if( i % 1000 == 0 ) {
      chprintf(chp, ".");
    }
    fr = f_write(&sd_file_handle, "test 4 5 6\n", 11, (UINT*)&bytes_written);
    if( fr != FR_OK || bytes_written != 11 )
    {
      chprintf(chp, "Failed to write to test.txt, %d\r\n", fr);
      goto sd_test_cleanup;
    }
  }
  chprintf(chp, "\r\n");
  ...
  fr = f_close(&sd_file_handle);
  if( fr != FR_OK )
  {
    chprintf(chp, "Error closing file, %d\r\n", fr);
  }

sd_test_cleanup:
  f_mount(0, NULL);
  sdcDisconnect(&SDCD1);
  palSetPad(GPIOC,13); // power off card
...
int main(void) {
...
#ifdef ENABLE_VCOM
  chThdCreateStatic(WA_sdcard_thread, sizeof(WA_sdcard_thread), NORMALPRIO, sdcard_thread_sdio_test, NULL);
  while (TRUE) {
    chThdSleepMilliseconds(100);
  }
#endif
  ...
  while (TRUE) {
    ...
    if (palReadPad(GPIOA, GPIOA_WAKE_BUTTON) != 0) {
    }
