...
; 3  History
;====================================================================
; Id: START.ASM 4591 2010-02-01 14:56:13Z rlande 

#define VERSION  "1.61"
/*
Revision 1.61 4591 2010-02-01 14:56:13Z rlande 
- updated: Disclaimer
...
;;					+------> Valore originale:	CPU_40MHZ_CLKP2_16MHZ
...
#set	CLOCK_OPTIMIZATION   LOW_PLL_OPTIMIZATION 	; <<< set clock optimization mode
;					+------> Valore originale:	WAIT_STATE_OPTIMIZATION	
...
; 4.11  ROM Mirror configuration
;--------------------------------------------------------------------
; ATTENZIONE! Questa sezione deve essere in accordo con i settaggi
; del compilatore relativi alla definizione di un segmento CONST,
; altrimenti l'applicazione non funzione correttamente (ad es. non
; funziona più la sprintf( ... ).
; In pratica se nel compilatore ho impostato un segmento CONST a 
; partire da 0xFA8000 io devo impostare 
;	MIRROR BANK = 0xA e MIRROR_SIZE = MIRROR_32KB
;====================================================================
...
#set      MIRROR_BANK   0xA         ; <<< ROM Mirror bank, allowed entries: 0x0..0xF for the banks 0xF0..0xFF

#set      MIRROR_SIZE   MIRROR_32KB ; <<< ROM Mirror size
;				+-------> Valore originale:	MIRROR_24KB
...
#set     COPY_RAMCODE     OFF		; <<< disable RAMCODE section to
									; be copied from ROM to RAM
...
#set    BACKGROUND_DEBUGGING  ON 	; <<< ENABLE  Background Debugging MODE
;#set   BACKGROUND_DEBUGGING  OFF 	; <<< DISABLE Background Debugging MODE


#set      BDM_CONFIGURATION  B'000000000000000  ; <<< set BDM configuration
;                                    ||||||||++--- BdmUART
;                                    ||||||||      (0: A, 1: B, 2: C, 3: D)
;                                    ||||||++----- BdmSynchMode
;                                    ||||||        (0: Async., 1: Sync.
...
;        .SECTION  CODE_START, CODE, ALIGN=1
         .SECTION  CODE_START, CODE, LOCATE=H'F80100
