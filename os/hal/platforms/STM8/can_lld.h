...
/**
 * @file STM8/can_lld.h
 * @brief STM8 CAN subsystem low level driver header.
 * @addtogroup STM8_CAN
 * @{
 */
...
/**
 * @brief Maximum number of CAN filters.
 */
#define CAN_MAX_FILTERS             6
 ...
 * @details If set to @p TRUE the support for CAN is included.
 * @note The default is @p TRUE.
 */
#if !defined(USE_STM8_CAN) || defined(__DOXYGEN__)
#define USE_STM8_CAN               TRUE
#endif
...
typedef struct {
  ...
  union {
    uint16_t              cf_SID;      /**< @brief Standard identifier.*/
    uint32_t              cf_EID;      /**< @brief Extended identifier.*/
  } id;
  union {
...
/**
 * @brief CAN filter.
 * @note Refer to the STM8 reference manual for info about filters.
 */
typedef struct {
  /**
   * @brief Filter mode.
   * @note This bit represents the CAN_FMRx register bits associated to this
   *       filter:
   *       00 - High Mask, Low Mask
   *       01 - High Mask, Low List
   *       10 - High List, Low Mask
   *       11 - High List, High List
   */
  uint8_t  cf_mode:2;
  /**
   * @brief Filter scale.
   * @note This bit represents the CAN_FSR register bit associated to this
   *       filter:
   *       00 - 8-bit Filter Scale
   *       01 - 16/8-bit Filter Scale
   *       10 - 16-bit Filter Scale
   *       11 - 32-bit Filter Scale (cf_mode must be 00 or 11)
   */
  uint8_t  cf_scale:2;

  uint8_t  cf_filterid1;     /**< @brief Filter register 1 (identifier) */
  uint8_t  cf_filterid2;     /**< @brief Filter register 2 (identifier) */
  uint8_t  cf_filterid3;     /**< @brief Filter register 3 (identifier) */
  uint8_t  cf_filterid4;     /**< @brief Filter register 4 (identifier) */
  uint8_t  cf_filteridmask1; /**< @brief Filter register 5 (identifier/mask) */
  uint8_t  cf_filteridmask2; /**< @brief Filter register 6 (identifier/mask) */
  uint8_t  cf_filteridmask3; /**< @brief Filter register 7 (identifier/mask) */
  uint8_t  cf_filteridmask4; /**< @brief Filter register 8 (identifier/mask) */
} CANFilter;
...
typedef struct {
  ...
  uint8_t                  cc_dgr;
  /**
   * @brief CAN BTR1 register initialization data.
   * @note Some bits in this register are enforced by the driver regardless
   *       their status in this field.
   */
  uint8_t                  cc_btr1;
